...
// TODO: Using `update()` highlights the underlying DOM mutation. Can we import
// a wrapper function (i.e. `counter`) and call it instead?
//
// Also, without a unique selector when updating an element, every matching
// element on the page will be updated. Can we use the wrapper function to
...
// where
//
// const component = fn => element(fn.name, fn())
//
// See commit f11330e for a start on the definition of `element()`. Better
// rename it to avoid overloading the term.
//
// This introduces a difference between an "element" (static) and a "component",
// which will reload when called a second time.
//
// Alternate approach to explore: Placing components inside a shadow DOM tree
// would allow us to uniquely identify them. Perhaps the aforementioned
// `element` function can reference this to establish uniqueness.
