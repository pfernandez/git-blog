...
const attachSubtree = ({element, properties, childNodes}) =>
  ...
  assignProperties({...element, childNodes}, properties)
...
const prepare = (tagName, x, childNodes) =>
  ({element: {tagName},  // baseElement(tagName),
    ...isProperties(x)
...
// Experimenting with a wrapper or `createElement` that infers the element name.
export const component = (fn, ...rest) =>
  createElement(
    fn.name // already named
    || log(new Error().stack.match(/at (\S+)/g)[1].slice(3))  // inline name
    || import.meta.url.split('/').slice(-1)[0].split('.')[0],  // default export name
    ...rest)
